...
class ResNet152Helper(nn.Module):
    ...
    def __init__(self, nb_classes=10, divergence_idx=None, nb_branch=None, shift=0, 
        ...
        shift: int 
            used to ensure that the total number of 
            parameters in MoE is not larger than the 
            monolithic baseline.
        """
        super().__init__()
...
def get_stem(divergence_idx, nb_branch, nb_classes, shift, 
    ...
    stem = SubResNet152(stem, start_idx=0, end_idx=divergence_idx)
...
def get_branches(divergence_idx, nb_branch, nb_classes, 
    ...
    pruned_branches = []
    for i in range(nb_branch_create):
        branch = SubResNet152(branches[i], start_idx=divergence_idx, end_idx=56)
        branch.to(device)
...
def lookup_precomputed_shift(divergence_idx, nb_branch, nb_classes, 
                             shift_path="precomputed_shift_lookup_table.csv"):
    shift_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), shift_path)
