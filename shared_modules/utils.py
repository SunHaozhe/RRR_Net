...
from .data_augmentation_utils import RandAugmentIrttssStraugV1
...
def get_transform(img_size, data_mean, data_std, args):
    """
    According to the straug paper, unlike in object recognition 
    where the accuracy increases with the number of augmentations, 
    in STR the peak is between N=2 and N=4. 
    Figure 13 shows that N=3 is the best.
    """
    transform = dict()
    if not args.use_albumentations:
        base_transforms_list = [lambda x: x.convert("RGB"),
                                transforms.Resize([img_size, img_size]),
                                lambda x: x.convert(args.img_mode),
                                transforms.ToTensor(),
                                transforms.Normalize(mean=data_mean,
                                                     std=data_std)]

        transform["test"] = transforms.Compose(base_transforms_list)

        if args.data_augmentation == "none":
            train_transforms = base_transforms_list
        elif args.data_augmentation == "RandAugmentIrttssStraugV1":
            train_transforms = [lambda x: x.convert("RGB"),
                                transforms.Resize([img_size, img_size]),
                                RandAugmentIrttssStraugV1(N=3, prob=1.,
                                ...
                                transforms.Normalize(mean=data_mean,
                                                     std=data_std)]
        elif args.data_augmentation == "RandAugmentIrttssV1":
            train_transforms = [lambda x: x.convert("RGB"),
                                transforms.Resize([img_size, img_size]),
                                RandAugmentIrttssStraugV1(N=3, prob=1.,
                                                          random_seed=args.random_seed,
        ...
        transform["train"] = transforms.Compose(train_transforms)
    else:
        proba = 1
...
def get_dataloaders(args):
    ...
    if not args.use_micro_version:
        train_dataset, test_dataset = get_ICDAR_2003_character_datasets(transform, args)
        val_dataset = None
    else:
...
def get_ICDAR_2003_character_datasets(transform, args):
    if not args.use_micro_version:
        if args.training_set == "full_train_split":
        ...
        test_dataset = Icdar2003CharacterDataset(train_split=False,
                                                enforce_all_alphanumeric_classes=True,
                                                use_albumentations=args.use_albumentations,
        ...
        train_dataset = Icdar2003CharacterDataset(use_micro_version=args.use_micro_version,
                                                 micro_split="train",
                                                  use_albumentations=args.use_albumentations,
                                                transform=transform["train"],
                                                dataset_root=os.path.join(
                                                    os.pardir, "datasets",
                                                    "ICDAR_2003_character_dataset"))
        val_dataset = Icdar2003CharacterDataset(use_micro_version=args.use_micro_version,
                                                micro_split="val",
                                                use_albumentations=args.use_albumentations,
                                                transform=transform["test"],
                                                dataset_root=os.path.join(
                                                    os.pardir, "datasets",
                                                    "ICDAR_2003_character_dataset"))
        test_dataset = Icdar2003CharacterDataset(use_micro_version=args.use_micro_version,
                                                micro_split="test",
                                                 use_albumentations=args.use_albumentations,
                                                transform=transform["test"],
                                                dataset_root=os.path.join(
    ...
    def prune_micro_examples(self, 
        ...
        for class_ in items_per_class.keys():
            if self.micro_split == "train":
                pruned_items.extend(items_per_class[class_][:train_examples_per_class])
                ...
                pruned_items.extend(items_per_class[class_][:test_examples_per_class])
