...
def get_previous_checkpoint_name(args):
    if args.use_ImageNet_pretraining:
        suffix = "ImagenetInit"
    else:
        suffix = "RandomInit"
        
    if args.previous_nb_blocks is None:
        checkpoint_name = "tmp.pth"
    else:
        checkpoint_name = "{}_{}_{}_{}_{}.pth".format(
            args.previous_nb_blocks[0], args.previous_nb_blocks[1], 
            args.previous_nb_blocks[2], args.previous_nb_blocks[3],
            suffix)
    return checkpoint_name


def get_curr_checkpoint_name(args):
    if args.use_ImageNet_pretraining:
        suffix = "ImagenetInit"
    else:
        suffix = "RandomInit"
    checkpoint_name = "{}_{}_{}_{}_{}.pth".format(
        args.nb_blocks[0], args.nb_blocks[1], 
        args.nb_blocks[2], args.nb_blocks[3],
        suffix)
    return checkpoint_name
...
def initialize_new_model_using_the_old_one(args, model, 
    ...
    if args.nb_blocks != [1, 1, 1, 1]:
        previous_checkpoint_name = get_previous_checkpoint_name(args)
        previous_checkpoint_path = os.path.join(
            resnet152_less_blocks_checkpoint_dir, previous_checkpoint_name)
        
        assert os.path.exists(previous_checkpoint_path)
        
        print("Loading model from {}".format(previous_checkpoint_path))
        
        pretrained_model_state_dict = torch.load(previous_checkpoint_path)
        model = load_weights_to_unmatched_model(
            model, pretrained_model_state_dict, verbose=True, use_wandb=use_wandb)
        
    if not os.path.exists(resnet152_less_blocks_checkpoint_dir):
...
def parse_arguments():
    ...
    parser.add_argument("--previous_nb_blocks", type=str, default=None,
                        help="""comma-separated list of 4 integers. 
                        each element cannot surpass 3,8,36,3. Used to 
                        specify which previous configuration to use 
    ...
    if args.previous_nb_blocks is not None:
        args.previous_nb_blocks = [int(xx)
                                for xx in args.previous_nb_blocks.split(",")]
        assert len(args.previous_nb_blocks) == 4
        assert args.previous_nb_blocks[0] >= 1 and args.previous_nb_blocks[0] <= 3
        assert args.previous_nb_blocks[1] >= 1 and args.previous_nb_blocks[1] <= 8
        assert args.previous_nb_blocks[2] >= 1 and args.previous_nb_blocks[2] <= 36
        assert args.previous_nb_blocks[3] >= 1 and args.previous_nb_blocks[3] <= 3
...
def init_wandb(args, verbose=True):
    if args.wandb_project_name is None:
        project_name = "PickResnet152Blocks_v2"
    else:
...
if __name__ == "__main__":
    ...
    model = initialize_new_model_using_the_old_one(
    ...
    curr_checkpoint_path = os.path.join(resnet152_less_blocks_checkpoint_dir, 
                                        get_curr_checkpoint_name(args))
    print("Saving model to {}".format(curr_checkpoint_path))
    torch.save(model.state_dict(), curr_checkpoint_path)
